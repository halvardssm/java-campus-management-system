package nl.tudelft.oopp.building.repositories;
...
public interface BuildingRepository extends JpaRepository<Building, Long> {
    ...
    @Query("SELECT u FROM Building u WHERE u.location LIKE CONCAT('%',:location,'%') and u.name LIKE CONCAT('%',:name,'%') and u.open <= :open and u.closed >= :closed and u.closed>=u.open")
    List<Long> filterBuildingsOnLocationAndNameAndTimeList(@Param("location") String location, @Param("name") String name, @Param("open") LocalTime open, @Param("closed") LocalTime closed);
